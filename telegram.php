...
while (true) {
    if(function_exists('xdebug_is_enabled')) {
        if(xdebug_is_enabled()) {
            echo 'Memory usage : '.xdebug_memory_usage()."\r";
        }
    }
...
>>>>>>> a20d31f22d8effe4ac6c1fafcaa085548f1f07ab
    ...
    elseif(preg_match('/^\/paste/', strtolower($msg['text']))) {
        $api_paste_code=substr($msg['text'], 7);
    ...
    elseif (preg_match('/^\/base64_decode/', strtolower($msg['text']))) {
        $telegram->bot('sendMessage',
            ['chat_id' => $msg['chat']['id'],
            'reply_to_message_id' => $msg['message_id'],
            'text' => base64_decode(substr($msg['text'], 15))
            ]);
    }
    ...
    elseif (preg_match('/^\/base64_encode/', strtolower($msg['text']))) {
        $telegram->bot('sendMessage',
            ['chat_id' => $msg['chat']['id'],
            'reply_to_message_id' => $msg['message_id'],
            'text' => base64_encode(substr($msg['text'], 15))
            ]);
    }
    ...
    elseif (preg_match('/^\/hex/', strtolower($msg['text']))) {
        $telegram->bot('sendMessage',
            ['chat_id' => $msg['chat']['id'],
            'reply_to_message_id' => $msg['message_id'],
            'text' => $telegram->hex(substr($msg['text'], 5))
            ]);
    }
    ...
    elseif (preg_match('/^\/binhex/', strtolower($msg['text']))) {
        $telegram->bot('sendMessage',
            ['chat_id' => $msg['chat']['id'],
            'reply_to_message_id'=> $msg['message_id'],
            'text' => $telegram->hex(gzdeflate(substr($msg['text'], 8)))
            ]);
